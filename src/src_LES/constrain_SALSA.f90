MODULE constrain_SALSA
  ...
  CONTAINS
   ...
   SUBROUTINE SALSA_diagnostics
      ...
      INTEGER :: i,j,k,bb,bc,ba,s,sc,sa,str,end,nc,nn, mi, mi2
   ...
   ! Ghost species
       ...
       DO i = 3,nxp-2
          DO k = 1,nzp
             ...
             DO bc = ica%cur,fcb%cur
                ...
                mi = getMassIndex(ncld,bc,nn)    ! all compounds
                mi2 = getMassIndex(ncld,bc,nn-1) ! Dry aerosol part
   		...
   		IF ( zdh2o < MIN(0.7*cdcld,1.3e-5) .OR.     &
             ...
             DO bc = ira,fra
		mi = getMassIndex(nprc,bc,nn)    ! all
                mi2 = getMassIndex(nprc,bc,nn-1) ! Dry aerosol part
    ...
    END DO   ! j
      ...
      a_ra(:,:,:) = SUM(a_maerop(:,:,:,str:end),DIM=4)
