   ...
   SUBROUTINE init_stat(time, filprf, expnme, nzp)
    ...
    ELSE IF ( level >= 4 ) THEN
       ...
       DO e=1,8 ! With water
          ...
          IF (level>=4) THEN
             s2bool(nvar2+31:nvar2+36) = .TRUE. !SV gases
             s2bool(nvar2+44:nvar2+45) = .TRUE. !SV gas phase conc
            s2bool(nvar2+46) = .TRUE. !Aerosol water content
          ENDIF
  ...
  END SUBROUTINE ts_lvl5
   ...
   SUBROUTINE accum_lvl4(n1,n2,n3)
      USE mo_submctl, ONLY : in1a,in2b,fn2a,fn2b,     &
                             ...
                             nprc,nlim,prlim,spec,mno,mnh,avog
      USE grid, ONLY : bulkNumc, bulkMixrat, meanRadius, binSpecMixrat, &
                       a_ra,a_rc, a_srp, a_rp, a_rh,    &
                       a_naerop, a_ncloudp, a_nprecpp,a_gaerop
    ...
    ! Gas-phase NO3 mixing ratio
    CALL get_avg3(n1,n2,n3,a_gaerop(:,:,:,2)*mno/avog,a2(:,1))

    ! Gas-phase NH3 mixing ratio
    CALL get_avg3(n1,n2,n3,a_gaerop(:,:,:,3)*mnh/avog,a2(:,2))
    svctr_b(:,44:45) = svctr_b(:,44:45) + a2(:,1:2)

    ! aerosol water mixing ratio
    CALL get_avg3(n1,n2,n3,a_ra,a2(:,1))
    svctr_b(:,46) = svctr_b(:,46) + a2(:,1)
