  ...
  SUBROUTINE actInterst(kproma,kbdim,klev,prv,prs,temp)
    ...
    DO jj = 1, klev
       DO ii = 1, kbdim
          ...
          zdcstar = 2.*zkelvin/( 3.*( (prv(ii,jj)/prs(ii,jj))-1. ) )*1.1
          ...
          DO cb = ica%cur, fcb%cur
             ...
             !Vlo = aero(ii,jj,ab)%vlolim         ! Dry vol at low limit
             ...
             ELSE
                ...
                END IF
                !Vlom1 = aero(ii,jj,ab-1)%vlolim !Vim1*paero(ii,jj,ab-1)%vratiolo
             ...
             ELSE
                ...
                END IF
        !        Vlop1 = aero(ii,jj,ab+1)%vlolim !Vip1*paero(ii,jj,ab+1)%vratiolo
             ...
             Vwlo = MAX(V01 + zs1*Vlo, Vlo) ! not 0 but Vlo
             Vwhi = MAX(V02 + zs2*Vhi, Vhi) ! not 0 but Vhi


             if(Vwhi<Vwmid) Vwhi=(Vwmid/Vwlo)**0.33*Vwmid
             Vwhi=min(Vwhi,((Vwmid/Vwlo))**.67*Vwmid)
             ...
             ELSE
                ...
                IF (vcut < Vmid .OR. vcut > Vhi) THEN
                   ! Intersection volume outside the current size range -> set as the lower limit
                   vint2 = Vmid
                   vint2= max(Vmid,zvcstar/Vwhi*Vhi)
                ELSE
                   vint2 = vcut
                END IF
...
!limited so that size won't decrease too much

                if(any(Vact(1:7)>0)) Vact(:) = min( &
                     sum(aero(ii,jj,ab)%volc(1:7))-1.0001*aero(ii,jj,ab)%vlolim*(aero(ii,jj,ab)%numc-Nact), &
                     sum(Vact(1:7)))/sum(Vact(1:7))*Vact(:)
