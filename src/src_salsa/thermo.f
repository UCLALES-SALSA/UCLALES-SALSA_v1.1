...
C ======================================================================
C
      SUBROUTINE THERMO(T,MOLAL,pH2O,pHNO3,pNH3,aw)
...
C
     ...
     >              Vca(NCmax,NAmax),Vac(NAmax,NCmax)
      ...
      COMMON/S0LIDS/SOLID_R,SOLID_S,SOLID_N
C
C
...
2150  FORMAT(/1X,
     >'Error - read error encountered while reading the reference',/1X,
     >'numbers of s0lids to be checked for saturation in aerotr.dat.',
     >/1X,'Stop before problem ',I7,'.')
2151  FORMAT(/1X,
...
C
C    --- s0lids options (for saturation ratios) ---
     >   'ice=',4,                          'h2so4=',6, 
...
C                           
C
C   ..initialise number of s0lids for saturation ratio checking
      nSLD=1
C
C      ===================================
...
C       .. type_size (0=normal, 1=small)          
        ...
        ENDIF
C
C     ..assign s0lids whose saturation ratios are to be checked. The
...
C     -----------------
      DO 10 I=1,5
        DO 11 J=1,NSpmax
          ...
          IF(I.EQ.1) SatRatio_S0lids(J)=0.D0
11      CONTINUE
10    CONTINUE
C
C
...
C
C     -------------------------------
C     Assign Option=2 to those s0lids
...
C     ..assign saturation ratio to new variable
        SatRatio_S0lids(iSolid(I))=SRATIO(I)
3     CONTINUE
C
C
...
C     -- arguments --
     ...
     >        SatRatio_S0lids(NSmax),pGas(NGmax)
C
C   ---------------------------------------------------------------
...
C     | options for s0lids whose saturation ratios should |
C     | be checked.                                       |
C      ---------------------------------------------------
      DO 10 I=1,NSmax
...
C           ..put header above first ratio to be printed
              IF(ldum) WRITE(*,280)
              WRITE(*,290) SatRatio_S0lids(I),Names_Species_R(4,I)
              ldum=.FALSE.
            ENDIF
          ENDIF
35      CONTINUE
...
280   FORMAT(/1X,
     > '- Calculated saturation ratios of s0lids -',/2X,
...
C
C     ---------------------------------------
C    | If s0lids are present (from which the |
...
C <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
C
C ===================== S0LIDS AND GASES (Gibbs program) ===================
C
